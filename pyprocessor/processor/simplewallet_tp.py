    ...
    def apply(self, transaction, context):
        ...
        if operation == "setflow":
            if len(payload_list) == 3:
                ...
                LOGGER.info('passhost {}'.format(passhost))
            # def _setflow(self, context, username, password, edgehost)
            self._setflow(context, userhost, passhost, edgehost)
            ...
            self._add_device(context, userhost, passhost, edgehost)
        elif operation == "init_edge":
            # _init_edge(self, context, edgehost)
            self._init_edge(context, edgehost)
        else:
